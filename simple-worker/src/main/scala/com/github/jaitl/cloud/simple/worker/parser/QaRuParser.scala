...
class QaRuParser extends BaseParser[StackowerflowParsedData] {
  ...
  override def parse(crawlTask: CrawlTask, crawlResult: CrawlResult): ParseResult[StackowerflowParsedData] = {
    ...
    val comments = doc.select("div.answers div.answer").asScala.map {
      el =>

        SatckoverflowComments(
          el.select("div.desc").html(),
          el.attr("id").toLong,
          dateFormat.parse(el.select("div.action-time span.hidden")
            .attr("datetime")
          ).getTime,
          SatckoverflowUser(
            el.select("div.action-time a").attr("href").split("/")
    ...
    val user = doc.select("div.action-time")
      .asScala.map {
      el =>
